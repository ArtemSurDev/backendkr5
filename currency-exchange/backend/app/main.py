...
if sys.version_info >= (3, 12):
    _orig_forwardref_evaluate = ForwardRef._evaluate

    def _forwardref_evaluate(self, globalns, localns, *args, **kwargs):
        type_params = None
        recursive_guard = kwargs.pop("recursive_guard", None)

        if args:
            if recursive_guard is None and isinstance(args[0], (set, frozenset)):
                recursive_guard = args[0]
            else:
                type_params = args[0]
                if recursive_guard is None and len(args) > 1 and isinstance(args[1], (set, frozenset)):
        ...
        return _orig_forwardref_evaluate(self, globalns, localns, type_params, recursive_guard=recursive_guard)

    ForwardRef._evaluate = _forwardref_evaluate
